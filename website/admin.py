...
def _build_menu_inventory_rows():
    master_products = (
        ProductMaster.query
    ...
    for product in master_products:
        canonical_name = (product.description or '').strip()
        normalized_name = _normalize_product_text(canonical_name)
        if not canonical_name or not normalized_name:
            continue

        bucket = inventory_map.get(normalized_name)
        if not bucket:
            bucket = {
                'product_name': canonical_name,
                'category': (product.category or '').strip() or 'Uncategorized',
                'alias_names': set(),
            }
            inventory_map[normalized_name] = bucket

        for alias in (product.aliases or []):
            alias_name = (alias.alias_name or '').strip()
            if alias_name:
                bucket['alias_names'].add(alias_name)

    alias_lookup = _get_product_alias_lookup()
    pos_rows = (
        db.session.query(PosSold.product_name)
    ...
    pos_only_map = {}
    for row in pos_rows:
        raw_name = (row.product_name or '').strip()
        if not raw_name or _is_grand_total_product_name(raw_name):
            continue

        canonical_name = alias_lookup.get(_normalize_product_text(raw_name), raw_name)
        canonical_name = (canonical_name or '').strip() or raw_name
        canonical_normalized_name = _normalize_product_text(canonical_name)
        if not canonical_normalized_name:
            continue

        if canonical_normalized_name in inventory_map:
            continue

        extra_bucket = pos_only_map.get(canonical_normalized_name)
        if not extra_bucket:
            extra_bucket = {
                'product_name': canonical_name,
                'raw_names': set(),
            }
            pos_only_map[canonical_normalized_name] = extra_bucket

        extra_bucket['raw_names'].add(raw_name)

    inventory_rows = []
    for normalized_name, payload in inventory_map.items():
        alias_names = sorted(payload['alias_names'], key=lambda name: name.lower())
    ...
    pos_only_rows = []
    for normalized_name, payload in pos_only_map.items():
        raw_names = sorted(payload['raw_names'], key=lambda name: name.lower())
        alias_names = [
            name for name in raw_names
            if _normalize_product_text(name) != _normalize_product_text(payload['product_name'])
        ]
        pos_only_rows.append({
            'product_name': payload['product_name'],
            'normalized_name': normalized_name,
    ...
    merged_rows = sorted(
        inventory_rows + pos_only_rows,
        key=lambda item: ((item.get('product_name') or '').lower(),),
    )
    return merged_rows, len(inventory_rows)
...
@admin.route('/admin/menu-inventory')
@login_required
def menu_inventory():
    if current_user.role not in ('Superadmin', 'Admin'):
        flash('Access denied.', category='error')
    ...
    merged_rows_all, total_products_master = _build_menu_inventory_rows()
    total_inventory_db = int(MenuInventoryItem.query.count() or 0)
    saved_names = {
        str(normalized_name or '').strip()
        for (normalized_name,) in MenuInventoryItem.query.with_entities(MenuInventoryItem.normalized_product_name).all()
        if str(normalized_name or '').strip()
    }
    pending_new_rows = [
        row for row in merged_rows_all
        if str(row.get('normalized_name') or '').strip() not in saved_names
    ]
    pending_new_count = len(pending_new_rows)
    pending_new_uncategorized_count = len([
        row for row in pending_new_rows
        if (row.get('category') or '').strip().lower() == 'uncategorized'
    ])
    merged_rows = list(merged_rows_all)
    full_uncategorized = len([
        row for row in merged_rows_all
        if (row.get('category') or '').strip().lower() == 'uncategorized'
    ])
    full_with_aliases = len([
        row for row in merged_rows_all
        if int(row.get('alias_count') or 0) > 0
    ])

    if q:
        q_norm = _normalize_product_text(q)
        merged_rows = [
            row for row in merged_rows
            if q_norm in _normalize_product_text(row['product_name'])
            or q_norm in _normalize_product_text(row['category'])
            or any(q_norm in _normalize_product_text(alias_name) for alias_name in row['alias_names'])
        ]
    ...
    summary = {
        'total_inventory': total_inventory_db,
        'total_products': total_products_master,
        'uncategorized': full_uncategorized,
        'with_aliases': full_with_aliases,
        'new_items_detected': pending_new_count,
        'pending_new': pending_new_count,
        'pending_new_uncategorized': pending_new_uncategorized_count,
    }

    if (request.args.get('ajax') or '').strip() == '1':
        return jsonify({
            'rows': merged_rows,
            'summary': summary,
        })
    ...
    return render_template(
        'admin/menu_inventory.html',
        user=current_user,
        search_query=q,
        inventory_rows=merged_rows,
        summary=summary,
        has_new_inventory=bool(pending_new_count > 0),
    )


@admin.route('/admin/menu-inventory/update', methods=['POST'])
@login_required
def menu_inventory_update():
    if current_user.role not in ('Superadmin', 'Admin'):
        flash('Access denied.', category='error')
        return redirect(url_for('views.home'))

    try:
        merged_rows, _ = _build_menu_inventory_rows()
        existing_items = MenuInventoryItem.query.all()
        existing_by_normalized = {
            str(item.normalized_product_name or '').strip(): item
            for item in existing_items
            if str(item.normalized_product_name or '').strip()
        }
        ...
        for row in merged_rows:
            ...
            is_changed = False
            if (existing.product_name or '') != product_name:
                existing.product_name = product_name
                is_changed = True
            if (existing.category or '') != category:
                existing.category = category
                is_changed = True
            if (existing.aliases_text or '') != aliases_text:
                existing.aliases_text = aliases_text
                is_changed = True
            if (existing.source or '') != source:
        ...
        log_audit_event(
            action='admin.menu_inventory.update',
            entity_type='MenuInventoryItem',
            entity_id='bulk',
            reason='Synced menu inventory list to DB.',
            details={
                'created_count': created_count,
                'updated_count': updated_count,
                'total_synced_rows': len(touched_normalized),
            },
        )
        db.session.commit()
        flash(
            f'Menu inventory updated. New: {created_count}, Updated: {updated_count}.',
            category='success',
        )
    except Exception as exc:
        db.session.rollback()
        flash(f'Error updating menu inventory: {str(exc)}', category='error')

    return redirect(url_for('admin.menu_inventory'))

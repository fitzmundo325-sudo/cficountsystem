...
function loadSystemAnalyzerData(startDate, endDate, presetLinkAlias) {
   ...
   let requestUrl =
      systemAnalyzerApiUrl + (params.toString() ? "?" + params.toString() : "");
   ...
   fetch(requestUrl, {
      ...
      .then(function (result) {
         if (result.type !== "success") {
            toast.error(
               result.message || "Failed to load system analyzer data.",
            );
            return;
         }
...
function updateDateRangeLabels() {
   _("pos-date-range-label").textContent =
      `Date Range: ${systemAnalyzerData.start_date} to ${systemAnalyzerData.end_date}`;
   _("rso-date-range-label").textContent =
      `Delivery and Bulk Order RSO · ${systemAnalyzerData.start_date} to ${systemAnalyzerData.end_date}`;
}
...
function buildPosRow(item) {
   ...
   row.querySelector('[data-cell="product_code"]').textContent =
      item.product_code || "-";
   row.querySelector('[data-cell="product_name"]').textContent =
      item.product_name || "-";
   row.querySelector('[data-cell="total_qty"]').textContent = item.total_qty;
   row.querySelector('[data-cell="total_net_sales"]').textContent =
      formatCurrency(item.total_net_sales);
   row.querySelector('[data-cell="store_count"]').textContent =
      item.store_count;
   row.querySelector('[data-cell="entry_count"]').textContent =
      item.entry_count;
   row.querySelector('[data-cell="latest_report_date"]').textContent =
      item.latest_report_date || "-";
   row.querySelector(".open-upload-modal").setAttribute(
      "data-product",
      item.product_name,
   );
   row.querySelector(".open-link-modal").setAttribute(
      "data-alias",
      item.product_name,
   );
   row.querySelector(".search-masterlist-link").href =
      masterlistBaseUrl + "?q=" + encodeURIComponent(item.product_name);
   return row;
}
...
function buildRsoRow(item) {
   ...
   row.querySelector('[data-cell="product_name"]').textContent =
      item.product_name || "-";
   row.querySelector('[data-cell="total_qty"]').textContent = item.total_qty;
   row.querySelector('[data-cell="total_received_qty"]').textContent =
      item.total_received_qty;
   row.querySelector('[data-cell="store_count"]').textContent =
      item.store_count;
   row.querySelector('[data-cell="entry_count"]').textContent =
...
function renderPosTable() {
   ...
   let filteredItems = getFilteredItems(
      systemAnalyzerData.unmatched_items,
      searchTerm,
   );
   ...
   _("pos-next-page").disabled =
      posCurrentPage >= totalPages || totalItems === 0;
}
...
function renderRsoTable() {
   ...
   let filteredItems = getFilteredItems(
      systemAnalyzerData.unmatched_rso_items,
      searchTerm,
   );
   let totalItems = filteredItems.length;
   let totalPages = Math.max(1, Math.ceil(totalItems / rowsPerPage));
   ...
   if (systemAnalyzerData.unmatched_rso_items.length === 0) {
      showEmptyMessage(
         tbody,
         "No unmatched RSO products detected for this date range.",
      );
   } else if (totalItems === 0) {
   ...
   _("rso-next-page").disabled =
      rsoCurrentPage >= totalPages || totalItems === 0;
}
...
function openUploadDetailsModal(source, productName) {
   ...
   let detailsMap = isRso
      ? systemAnalyzerData.rso_upload_details
      : systemAnalyzerData.pos_upload_details;
   ...
   if (!rows.length) {
      body.innerHTML =
         '<div class="rounded-xl border border-slate-200 bg-slate-50 px-4 py-8 text-center text-sm text-slate-500">No upload details found for this product in the selected date range.</div>';
   } else {
      let headers = isRso
         ? [
              "Store",
...
function openLinkProductModal(aliasValue) {
   ...
   let masterInput = document.querySelector(
      'input[name="master_product_name"]',
   );
...
function handleLinkProductSubmit(event) {
   ...
   if (!aliasValue || !masterValue) {
      toast.error(
         "Please fill the detected name and master product name first.",
      );
      return;
   }
...
function handleGlobalTableActionClick(event) {
   ...
   if (uploadBtn) {
      ...
      openUploadDetailsModal(
         uploadBtn.getAttribute("data-source") || "pos",
         uploadBtn.getAttribute("data-product") || "",
      );
      return;
   }
...
function initializeSystemAnalyzer() {
   let params = new URLSearchParams(window.location.search);
   loadSystemAnalyzerData(
      params.get("start_date") || "",
      params.get("end_date") || "",
      params.get("link_alias") || "",
   );
}
...
_("upload-details-backdrop").addEventListener(
   "wheel",
   preventBackdropWheelScroll,
   { passive: false },
);
...
_("link-product-backdrop").addEventListener(
   "wheel",
   preventBackdropWheelScroll,
   { passive: false },
);
